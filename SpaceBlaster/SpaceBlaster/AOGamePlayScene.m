...
/*
 To do:
  -Tabla de scores local(mientras no hay en game center)
 -game center
 -boton de desactivar audio
 -boton pausa
 -gameplay music
 -refactor code
 -corregir lo de scores
...
@property (nonatomic) BOOL isShapeShipReady;
...
-(void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (!self.gameOver) {
        ...
        if (self.isBulletReady && self.bulletsLeft > 0 &&touchedNode != self && ![touchedNode.name  isEqual: @"Ammo"] && self.isShapeShipReady) {
        ...
        if (touchedNode != self && [touchedNode.name  isEqual: @"Restart"]) {
            ...
            [[AOGameData sharedGameData] reset];
        ...
        if (touchedNode != self && [touchedNode.name  isEqual: @"Share"]) {
            ...
            [composer setInitialText:[NSString stringWithFormat:@"Just Scored %ld points on Rock Blaster!!! \n #RockBlasters",[AOGameData sharedGameData].score]];
...
-(void)didBeginContact:(SKPhysicsContact *)contact{
    ...
    //Hubo colision entre un meteoro y la nave
              ...
              secondBody.categoryBitMask == AOCollissionCategoryShip && !spaceShip.isInmune){
        //self.isShapeShipReady = NO;
        spaceShip.isInmune = YES;
        ...
        SKAction *blinkForTime = [SKAction repeatAction:blink count:8];
        
        [spaceShip runAction:blinkForTime completion:^{
            spaceShip.isInmune = NO;
        }];
...
-(void) addPoints:(NSInteger)points{
    ...
    [AOGameData sharedGameData].highScore = MAX([AOGameData sharedGameData].score,
                                                [AOGameData sharedGameData].highScore);
...
-(void) performGameOver{
    ...
    SKAction *waiting = [SKAction waitForDuration:1.0];
    
    [self runAction:waiting completion:^{
        [self setUserInteractionEnabled: YES];
    }];
    
    [[AOGameData sharedGameData] save];
    //[[AOGameData sharedGameData] reset];
...
-(void)createDebrisAtPosition:(CGPoint) position{
    ...
    for (int i = 0; i < numberOfPieces; i++) {
        NSInteger randomPiece = [AOUtil randomWithMin:1 max:5];
